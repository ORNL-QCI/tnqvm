...
const std::string noise_model_multi_depol =
...
const std::string noise_model_weak_noise =
...
const std::string noise_model_4q =
...
TEST(JsonNoiseModelTester, checkSimple) {
  // Check depolarizing channels
  {
    ...
    // Check trace
    EXPECT_NEAR(densityMatrix[0][0].real() + densityMatrix[1][1].real(), 1.0,
                1e-6);
    // Expected result:
...
TEST(JsonNoiseModelTester, checkBitOrdering) {
  ...
  std::vector<std::vector<std::complex<double>>> densityMatrix_msb,
      densityMatrix_lsb;
  // Check MSB:
  {
    auto noiseModel = xacc::getService<xacc::NoiseModel>("json");
...
TEST(JsonNoiseModelTester, checkOutputLegOrder) {
  std::vector<std::vector<std::complex<double>>> densityMatrix_no_noise,
      densityMatrix_weak_noise;
  auto xasmCompiler = xacc::getCompiler("xasm");
  auto program = xasmCompiler
...
TEST(JsonNoiseModelTester, checkGHZ) {
  ...
  // Expect the first and last element of the matrix are near 0.5 (GHZ state)
  const auto dmDimension = 1ULL << buffer->size();
  EXPECT_NEAR(densityMatrix[0][0].real(), 0.5, 0.1);
  EXPECT_NEAR(densityMatrix[dmDimension - 1][dmDimension - 1].real(), 0.5, 0.1);
  EXPECT_NEAR(densityMatrix[0][0].imag(), 0.0, 1e-6);
  EXPECT_NEAR(densityMatrix[dmDimension - 1][dmDimension - 1].imag(), 0.0,
              1e-6);
}

// Test VQE with noise
TEST(JsonNoiseModelTester, testDeuteronVqeH2) {
  auto noiseModel = xacc::getService<xacc::NoiseModel>("json");
  noiseModel->initialize({{"noise-model", noise_model_4q}});
  auto accelerator = xacc::getAccelerator(
      "tnqvm", {{"tnqvm-visitor", "exatn-dm"}, {"noise-model", noiseModel}});
  ...
  auto ansatz = xacc::getCompiled("deuteron_ansatz1");
